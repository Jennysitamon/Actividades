...
function createPagination() {
    let buttons = `<li class="page-item"><a class="page-link" href="#">Previous</a></li>`;
    for (let i = 1; i < 42; i++) {
...
function createCardChar(personaje) {
    ...
    if (personaje.status == 'Alive') {
        ...
        badge = `<span class="badge rounded-pill text-bg-warning">${personaje.status}</span>`
    }
    ...
    <div class="card-body">
      ...
      <p class="card-text">${personaje.location}</p>
    </div>`;
    card.innerHTML = cardContent;
    return card;
}
